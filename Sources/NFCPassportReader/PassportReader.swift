...
extension NFCViewDisplayMessage {
    public var description: String {
        switch self {
        case .requestPresentPassport:
            return "Yeni Çipli T.C. Kimlik Kartınızı cihazınızın arkasında tutunuz."
        case .authenticatingWithPassport(let progress):
            let progressString = handleProgress(percentualProgress: progress)
            return "Belge doğrulaması yapılmaktadır, lütfen bekleyiniz.....\n\n\(progressString)"
        case .readingDataGroupProgress(let dataGroup, let progress):
            let progressString = handleProgress(percentualProgress: progress)
            return "Lütfen kartınızı hareket ettirmeyiniz.\n\n\(progressString)"
        case .error(let tagError):
            switch tagError {
            case TagError.TagNotValid:
                return "Maalesef kimlik kartı okunurken bir sorun oluştu."
            ...
            case TagError.InvalidMRZKey:
                return "Hatalı kimlik bilgisi. Kimlik bilgilerinin doğruluğuna emin olunuz."
...
public class PassportReader : NSObject {
    ...
    public func readPassport( challenge:[UInt8], mrzKey : String, tags: [DataGroupId] = [], skipSecureElements :Bool = true, customDisplayMessage: ((NFCViewDisplayMessage) -> String?)? = nil, completed: @escaping (NFCPassportModel?, TagError?)->()) {
